...
function createMemoryId(prefix, content) {
...
function normalizeRecord(row) {
    return {
        ...
        tags: safeJsonParse(typeof row.tags === 'string' ? row.tags : '', []),
        ...
        metadata: row.metadata == null
            ? null
            : safeJsonParse(typeof row.metadata === 'string' ? row.metadata : '', null),
...
export class MemoryStore {
    ...
    async append(input) {
        ...
        if (this.backend === 'sqlite' && this.db) {
            const statement = this.db.prepare(`INSERT INTO memories (
          id, kind, scope, content, tags, confidence, source_ref,
          session_id, metadata, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
            statement.run(record.id, record.kind, record.scope, record.content, JSON.stringify(record.tags), record.confidence, record.sourceRef, record.sessionId, record.metadata ? JSON.stringify(record.metadata) : null, record.createdAt, record.updatedAt);
    ...
    async search(query = {}) {
        await this.ensureInitialized();
        const limit = Math.min(Math.max(1, query.limit ?? this.config.maxSearchResults), this.config.maxSearchResults);
        if (this.backend === 'sqlite' && this.db) {
            return this.searchSqlite(query, limit);
        }
        return this.searchFile(query, limit);
    }
    async compact(summary, options = {}) {
        return this.append({
    ...
    async searchSqlite(query, limit) {
        if (!this.db) {
            return [];
        }
        const clauses = [];
    ...
    async cleanupOverflowIfNeeded() {
        ...
        const serialized = trimmed
            ...
            .join('\n');
        await fs.writeFile(this.fileStorePath, serialized.length > 0 ? `${serialized}\n` : '', 'utf-8');
    }
}

...
export class CommentSanitizer {
   ...
   */
  async sanitizeFile(filePath: string, dryRun: boolean = true): Promise<SanitizerResult> {
  ...
  private getCommentPatterns(ext: string): { single?: string; multiStart?: string; multiEnd?: string } | null {
    switch (ext) {
      case '.ts':
      case '.tsx':
      case '.js':
  ...
  private isSingleLineComment(line: string, prefix: string): boolean {
    return line.startsWith(prefix);
  }

  private shouldPreserveSingleLine(line: string): boolean {
  ...
  private shouldPreserveMultiline(comment: string): boolean {
  ...
  private processInlineComment(
    line: string,
    prefix: string
  ): { hadComment: boolean; removed: boolean; newLine: string } {
...
let sanitizerInstance: CommentSanitizer | null = null;

export function getCommentSanitizer(): CommentSanitizer {
...
export function createCommentSanitizer(config?: Partial<SanitizerConfig>): CommentSanitizer {
  return new CommentSanitizer(config);
}

// Convenience function
export async function sanitizeComments(
  target: string,
  options: {
    dryRun?: boolean;
    recursive?: boolean;

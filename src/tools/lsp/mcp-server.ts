...
class LSPManager {
  ...
  private defaultRootPath: string = process.cwd();
 ...
 */
export async function lspGotoDefinition(params: GotoDefinitionParams): Promise<LSPToolResult> {
  try {
 ...
 */
export async function lspFindReferences(params: FindReferencesParams): Promise<LSPToolResult> {
  try {
 ...
 */
export async function lspWorkspaceSymbols(params: WorkspaceSymbolsParams): Promise<LSPToolResult> {
  try {
 ...
 */
export async function lspDocumentSymbols(params: DocumentSymbolsParams): Promise<LSPToolResult> {
  try {
    const { file } = params;
    const absolutePath = path.resolve(file);
 ...
 */
export async function lspRename(params: RenameParams): Promise<LSPToolResult> {
  try {
 ...
 */
export async function lspHover(params: HoverParams): Promise<LSPToolResult> {
  try {
    const { file, line, character } = params;
    const absolutePath = path.resolve(file);
 ...
 */
export async function lspDiagnostics(params: DiagnosticsParams): Promise<LSPToolResult> {
  try {
...
function formatLocations(locations: Location[], type: string): string {
...
function formatSymbols(symbols: SymbolInformation[]): string {
...
function formatDocumentSymbols(symbols: DocumentSymbol[] | SymbolInformation[], indent: number = 0): string {
  const lines: string[] = [];
...
function formatWorkspaceEdit(edit: WorkspaceEdit): string {
  const lines: string[] = ['Rename will affect the following files:\n'];
...
function summarizeWorkspaceEdit(edit: WorkspaceEdit): { files: number; changes: number } {
  let files = 0;
  let changes = 0;
...
function formatDiagnostics(diagnostics: Diagnostic[], file: string): string {
  const lines = [`Diagnostics for ${file}:\n`];

  const severityIcon: Record<number, string> = {

...
function estimateTokensBySize(size: number): number {
  if (size <= 0) return 0;
  return Math.ceil(size / 4);
}

async function statTokenEstimate(fullPath: string): Promise<number> {
  try {
...
function normalizeKeywordQuery(taskDescription?: string): string {
...
async function collectGlobalRefs(projectRoot: string): Promise<ContextReference[]> {
...
async function collectSprintRefs(
  projectRoot: string,
  maxEntries: number
  ...
  const ranked: Array<{ path: string; score: number; mtimeMs: number; size: number }> = [];
  ...
  for (const relPath of candidatePaths) {
    ...
    try {
      ...
      let score = 0;
      if (lower.includes('sprint')) score += 5;
      if (lower.includes('packet')) score += 4;
      if (lower.includes('spec')) score += 3;
      if (lower.includes('openapi')) score += 3;
      if (lower.includes('prd')) score += 2;
      if (lower.includes('architecture')) score += 2;
...
async function collectExecutionRefs(
  projectRoot: string,
  workingFiles: string[],
  maxEntries: number
): Promise<ContextReference[]> {
...
async function collectMemorySlice(
  projectRoot: string,
...
function trimReferencesToBudget(
...
export async function buildContextPack(options: ContextPackOptions): Promise<ContextPack> {
  ...
  const executionRefs = await collectExecutionRefs(
    options.projectRoot,
    workingFiles,
    maxLayerEntries
  );

  const memory = await collectMemorySlice(
    options.projectRoot,
...
export function formatContextPackMarkdown(pack: ContextPack): string {
  ...
  const renderLayer = (name: string, refs: ContextReference[]) => {
    lines.push(`### ${name}`);

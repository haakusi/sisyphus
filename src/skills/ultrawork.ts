...
function detectRequestedFeatures(message: string): Partial<UltraworkFeatures> {
  const features: Partial<UltraworkFeatures> = {};
...
export const ultraworkSkill: Skill = {
  name: 'ultrawork',
  aliases: ['ulw', 'ultra'],
  ...
  async execute(context: SkillContext): Promise<SkillResult> {
    ...
    const features: UltraworkFeatures = {
...
function buildUltraworkInstructions(
  features: UltraworkFeatures,
...
export function isUltraworkRequest(message: string): boolean {
  const lowerMessage = message.toLowerCase();

  return ultraworkSkill.triggers.some((trigger) =>

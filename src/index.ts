...
async function refreshCompactionSnapshot(ctx: PluginContext, reason: string): Promise<void> {
  try {
...
function initializeSisyphus(ctx: PluginContext): void {
  ...
  if (config.artifactDrift?.enabled !== false) {
    void artifactDriftStatus({ projectRoot: ctx.projectRoot }).then((driftResult) => {
      ...
      const report = driftResult.data as { issues?: Array<{ severity: string }> } | undefined;
      const issueCount = report?.issues?.length ?? 0;
      const highCount =
        report?.issues?.filter((issue) => issue.severity === 'high').length ?? 0;
  ...
  void policyActiveRules({ projectRoot: ctx.projectRoot, limit: 10 }).then((policyResult) => {
    ...
    const manifest = policyResult.data as { rules?: unknown[] } | undefined;
